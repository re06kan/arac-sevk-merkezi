...
// KM güncelleme endpoint'i (hem başlangıç hem bitiş KM değerlerini günceller)
router.put('/:id/km', async (req, res) => {
  try {
    const taskId = req.params.id;
    const { start_km, end_km, reason } = req.body;

    // Task'ı veritabanından bul
    ...
    // KM değerlerini ve güncelleme sebebini kaydet
    const result = await pool.query(
      `UPDATE task_registration
       SET start_km = $1, end_km = $2, km_update_reason = $3, modified_at = CURRENT_TIMESTAMP
       WHERE id = $4
       RETURNING *`,
      [start_km, end_km, reason, taskId]
    );

    return res.status(200).json({
      success: true,
      message: 'KM bilgisi başarıyla güncellendi',
      task: result.rows[0]

    });
  } catch (error) {
    console.error('KM güncelleme hatası:', error);
    return res.status(500).json({ error: 'KM güncelleme işlemi sırasında bir hata oluştu' });
  }
});
